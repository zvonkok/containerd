...
func initCRICCService(ic *plugin.InitContext) (interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get caps: %w", err)
	}
	cc := &ccService{
		Store:     criStore,
...
func (cc *ccService) CreateContainer(ctx context.Context, r *runtime.CreateContainerRequest) (_ *runtime.CreateContainerResponse, retErr error) {
	...
	// CC: Skip preparing container image snapshot.
	...
	// CC: Skip c.config.IgnoreImageDefinedVolumes
	...
	log.G(ctx).Debugf("CC: Use OCI runtime %+v for sandbox %q and container %q", ociRuntime, sandboxID, id)
	...
	log.G(ctx).Debugf("CC: Container %q spec: %#+v", id, spew.NewFormatter(spec))

	// CC: Skip snapshotterOpt
	...
	// CC: Use const container kind
	const containerKindContainer = "container"
	containerLabels := server.BuildLabels(config.Labels, nil, containerKindContainer)
...
func (cc *ccService) containerSpec(
	...
	specOpts = append(specOpts, customopts.WithMounts(cc.os, config, extraMounts, mountLabel))
...
func (cc *ccService) validateTargetContainer(sandboxID, targetContainerID string) (containerstore.Container, error) {
	...
	if state := status.State(); state != runtime.ContainerState_CONTAINER_RUNNING {
		return containerstore.Container{},
			fmt.Errorf("container %q is not running - in state %s", targetContainerID, state)
	}
